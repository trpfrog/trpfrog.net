...
async function fetchTweet(tweetUrl: string): Promise<FetchedTweet> {
...
function createTweetBlock(result: FetchedTweet): string {
...
export const route = new Hono().post(
  ...
  async c => {
    try {
      ...
      const codeBlock = createTweetBlock(result)
