...
const createArticleStyle = tv({
  slots: {
    ...
    info: 'tw:flex tw:w-2/3 tw:flex-col tw:text-white tw:sp:w-3/4',
    button: cardButtonStyle(),
  },
  variants: {
    rightToLeft: {
      true: {
        wrapper: 'tw:flex-row-reverse tw:bg-linear-to-l',
        info: 'tw:items-end tw:text-right',
      },
      false: {
        wrapper: 'tw:bg-linear-to-r',
      },
    },
    index: {
      0: {
        wrapper: `tw:from-pink-500 tw:to-pink-500/40 tw:dark:from-pink-900 tw:dark:to-pink-900/40`,
        button: 'tw:translate-y-3',
      },
      1: {
        wrapper: `tw:from-amber-500 tw:to-amber-500/40 tw:dark:from-amber-900 tw:dark:to-amber-900/40`,
      },
      2: {
        wrapper: `tw:from-lime-600 tw:to-lime-600/40 tw:dark:from-lime-900 tw:dark:to-lime-900/40`,
      },
    },
  },
})
    ...
    >
      <div className={articleStyle.wrapper()}>
        ...
        <A href={`/blog/${entry.slug}`} className={articleStyle.button()}>
          <span className="tw:sp:hidden">記事を</span>読む
          <span className="tw:hidden tw:sp:inline"> &rarr;</span>
        </A>
      </div>
    </div>
  )
