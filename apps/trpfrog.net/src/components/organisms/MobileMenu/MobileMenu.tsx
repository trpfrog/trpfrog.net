...
import { useMobileMenuState, useToggleMenuCallback } from './hooks'
import { Settings } from './Settings'

function SpeechBubbleContainer({ children }: { children: React.ReactNode }) {
  ...
  return <aside className={styles}>{children}</aside>
}
...
export function MobileMenu() {
  ...
  return (
    <div inert={!isMenuOpen}>
      <MobileMenuBkg>
        <SpeechBubbleContainer>
          <PlainBlock className="tw-flex tw-flex-col tw-gap-3 tw-rounded-lg tw-p-4 sp:tw-w-full">
            <button className="tw-grid tw-grid-cols-2 tw-gap-x-2 tw-gap-y-2.5" onClick={toggleMenu}>
